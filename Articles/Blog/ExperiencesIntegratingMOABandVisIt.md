...
The design of the MOAB plugin for the VisIt visualization platform is motivated to create a bridge between MOAB’s underlying mesh data structure in order to exploit VisIt’s scalable rendering and analysis capabilities. 

MOAB's native (and preferred) unstructured mesh storage is based on the HDF5 format, where the entire distributed mesh is stored as a monolithic graph within a single file. This is in contrast to using multiple files written out from different tasks [REFERENCE - silo/vtk/zarr?]. The plugin, implemented through the `avtMOABFileFormat` class within VisIt’s plugin architecture, enables VisIt to directly load and visualize MOAB `h5m` files. Internally, the plugin leverages MOAB’s C++ API to extract the necessary mesh connectivity, topological adjacencies, and tagged field data, in order to translate them into VisIt’s `avtMesh` and `avtVariable` structures. The plugin also populates the necessary database metadata (__talk about load options here__), enabling full support for mesh (__dimension selection?__) and variable selection (__dimension based and sorted?__) dynamically at runtime through user specified options during file load.

To enable parallel read, the mesh needs to be prepartitioned into patches of entities (vertices and topological entities), which are internally referred to as "parts". MOAB supports the use of [Zoltan](https://sandialabs.github.io/Zoltan/) or [Metis](https://github.com/KarypisLab/METIS) partitioners and exposes both geometric and graph based strategies for optimal domain decomposition. The computed partition information is saved as a "tag" (typically named as the PARALLEL_PARTITION tag) in the mesh file with groups of entities as partition sets.
